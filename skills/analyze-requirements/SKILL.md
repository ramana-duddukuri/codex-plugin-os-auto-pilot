...
> **IMPORTANT — Output file location:** Always write ALL output files (`test-cases-*.md`, `test-coverage.md`, `swagger-spec.json`, etc.) to the **current working directory**. Never write files into the plugin directory or the skill's own folder. Use plain filenames (no path prefix) so they land in the cwd automatically.
