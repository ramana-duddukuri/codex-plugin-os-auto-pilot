...
> **IMPORTANT — File location:** All markdown files (`test-cases-*.md`, `test-coverage.md`) live in the **current working directory**. Always read from and write to the cwd, never the plugin directory.
...
**no LLM re-analysis** — it uses exactly what the model wrote, in exactly the order
